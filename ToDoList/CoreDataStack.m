...
static CoreDataStack *cds = nil;

+(CoreDataStack*) coreDataStack {
    if (!cds) {
        cds = [[CoreDataStack alloc] init];
    }
    return cds;
}

- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "com.kevinremigio.ToDoList" in the application's documents directory.
...
-(NSArray*)getAllItems {
    ...
    NSEntityDescription *entity = [NSEntityDescription entityForName:@"ToDoList" inManagedObjectContext:cds.managedObjectContext];
...
-(void)addItem:(NSString*)note {
    //CoreDataStack *cds = [CoreDataStack coreDataStack];
    ToDoList *tdl = [NSEntityDescription insertNewObjectForEntityForName:@"ToDoList" inManagedObjectContext:self.managedObjectContext];
    tdl.note = note;
    tdl.date = [NSDate date];
